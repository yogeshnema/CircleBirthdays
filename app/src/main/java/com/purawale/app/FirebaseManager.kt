...
object FirebaseManager {
    ...
    suspend fun sendMessage(message: Message) {
        ...
        val recipientSnapshot = db.collection("members").document(message.receiverId).get().await()
        val recipientToken = recipientSnapshot.getString("fcmToken")
        val senderSnapshot = db.collection("members").document(message.senderId).get().await()
        val senderName = senderSnapshot.getString("name") ?: "Someone"
    ...
    private suspend fun triggerNotification(
        ...
        val notificationRequest = mutableMapOf(
        ...
        toToken?.let { notificationRequest["to"] = it }
        topic?.let { notificationRequest["topic"] = it }
        senderId?.let { notificationRequest["senderId"] = it }

        db.collection("notifications").add(notificationRequest).await()
    }
    ...
    fun getPendingChanges(onResult: (List<Member>) -> Unit, onError: (Exception) -> Unit = {}): ListenerRegistration {
        return db.collection("pending_updates")
            .orderBy("familyId")
            .addSnapshotListener { snapshot, error ->
                if (error != null) {
    ...
    fun getRelationshipOverrides(onResult: (List<RelationshipOverride>) -> Unit): ListenerRegistration {
        return db.collection("relationship_overrides")
    ...
    suspend fun submitMemory(memory: Memory) {
        ...
        triggerNotification(
            ...
            body = "${memory.userName} shared a new memory!",
            type = "GALLERY"
        )
    }
    ...
    suspend fun submitRecipe(recipe: Recipe) {
        ...
        triggerNotification(
            ...
            body = "${recipe.authorName} added a recipe: ${recipe.title}",
            type = "RECIPE"
        )
    }
    ...
    fun getTraditions(onResult: (List<Tradition>) -> Unit): ListenerRegistration {
        return db.collection("traditions")
            .orderBy("timestamp")
            .addSnapshotListener { snapshot, error ->
                if (error != null) return@addSnapshotListener
    ...
    suspend fun submitTradition(tradition: Tradition) {
        ...
        triggerNotification(
            ...
            body = "${tradition.authorName} shared: ${tradition.title}",
            type = "TRADITION"
        )
    }
    ...
    fun getMilestones(onResult: (List<Milestone>) -> Unit): ListenerRegistration {
        return db.collection("memorylane")
            .orderBy("year")
            .addSnapshotListener { snapshot, error ->
                if (error != null) {
                    android.util.Log.e("FirebaseManager", "Error fetching milestones", error)
                    return@addSnapshotListener
                }
                val milestones = snapshot?.documents?.mapNotNull { doc ->
                    try {
                        ...
                        android.util.Log.e("FirebaseManager", "Error parsing milestone ${doc.id}", e)
                        null
                    }
                } ?: emptyList()
    ...
    suspend fun toggleMilestoneReaction(milestoneId: String, emoji: String, userId: String) {
        if (milestoneId.isBlank()) return
        db.runTransaction { transaction ->
            val docRef = db.collection("memorylane").document(milestoneId)
            val snapshot = transaction.get(docRef)
            val data = snapshot.data ?: return@runTransaction
    ...
    suspend fun addMilestoneComment(milestoneId: String, comment: Comment) {
        ...
        db.runTransaction { transaction ->
            ...
            if (data != null) {
                val commentsRaw = data["comments"] as? List<Map<String, Any>> ?: emptyList()
                val updatedComments = commentsRaw.toMutableList()
                updatedComments.add(mapOf(
                    "id" to comment.id,
